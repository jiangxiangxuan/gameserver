...
int main( int argc, char **argv )
{
	...
	DBServer::createInstance();
	DBServer::getInstance()->init("../config/config.xml");

	DBServer::getInstance()->run();

	DBServer::getInstance()->uninit();
	
	DBServer::destroyInstance();
...
void exceptionalStack(int signal)
{
	...
	pid_t pid = getpid();
	char cmd[128];
	sprintf(cmd, "gstack %d > core_db_server_log_%d.core", pid, nowtime);
	system(cmd);

	//DBServer::getInstance()->onExit();
	DBServer::getInstance()->uninit();
	exit( -1 );
}
